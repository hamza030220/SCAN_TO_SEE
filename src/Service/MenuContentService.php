...
final class MenuContentService
{
    ...
    private function copyItem(Item $source, ?Category $category): Item
    {
        return (new Item())
            ...
            ->setBadge($source->getBadge())
            ->setDietaryTags($source->getDietaryTags())
            ->setAllergens($source->getAllergens())
            ->setVariants($source->getVariants())
            ->setAvailabilityNote($source->getAvailabilityNote())
