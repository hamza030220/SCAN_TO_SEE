...
final class MenuThemeConfigService
{
    ...
    public function sanitize(array $input, array $current = []): array
    {
        ...
        return [
            ...
            'priceStyle' => $this->enum($input, 'priceStyle', ['accent', 'neutral', 'outline', 'badge'], $defaults['priceStyle']),
            'priceAlign' => $this->enum($input, 'priceAlign', ['left', 'center', 'right'], $defaults['priceAlign']),
            'priceFont' => $this->enum($input, 'priceFont', $this->fontCatalog->allowedFamilies(), $defaults['priceFont']),
            'priceSize' => min(1.4, max(0.65, round((float) ($input['priceSize'] ?? $defaults['priceSize']), 2))),
            'priceWeight' => $this->enum($input, 'priceWeight', ['500', '700', '800'], $defaults['priceWeight']),
            'priceColor' => $this->color($input, 'priceColor', $defaults['priceColor']),
            'priceBoxColor' => $this->color($input, 'priceBoxColor', $defaults['priceBoxColor']),
            'priceRadius' => min(20, max(0, (int) ($input['priceRadius'] ?? $defaults['priceRadius']))),
